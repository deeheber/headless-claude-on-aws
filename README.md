...
- **The two-role split is the real point of the setup.** Build role and agent role are deliberately separate so the agent can't read the build's secrets or use its CodeConnection. Same-account assume-role looks like overkill for a demo, but it's the same shape you'd use cross-account, so the same pattern would work there.
